...
class Authentication(usersFile: File) {
	...
	private fun decodeHeader(encodedHeader: String): String =
		Base64.getDecoder().decode(encodedHeader).toString(Charset.defaultCharset())
	...
	private fun isAuthenticated(credentials: Pair<String, String>): Boolean =
		users[credentials.first] == credentials.second
